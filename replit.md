...
- `index.html` has title, description, og tags, theme-color, robots, and `<link rel="sitemap">`.
...
- **`/sitemap.xml`** and **`/robots.txt`** are generated by `scripts/sitemap-plugin.ts` (custom Vite plugin):
  - Serves both files in dev via Vite middleware.
  - Emits both files into `dist/public/` on build via `generateBundle`.
  - Site URL defaults to `https://toolshub.app`. Override at build time with `SITE_URL=https://yourdomain.com pnpm --filter @workspace/toolshub run build`.
  - Sitemap covers home, about, contact, privacy, and all 7 tool pages with sensible `priority`/`changefreq` values.
